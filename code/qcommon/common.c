...
#ifdef Q3_VM
/*
 * Lightweight bump allocator for Q3 VM builds.
...
void *Z_Malloc( int size ) {
	void *p;
	size = (size + 7) & ~7;
	if ( vmZoneUsed + size > VM_ZONE_SIZE ) {
		Com_Error( ERR_DROP, "Z_Malloc: failed on allocation of %i bytes", size );
		return NULL;
	}
	p = vmZoneMem + vmZoneUsed;
	vmZoneUsed += size;
	return p;
}

void Z_Free( const void *ptr ) { (void)ptr; /* no-op with bump allocator */ }

int Z_AvailableMemory( void ) {
	return VM_ZONE_SIZE - vmZoneUsed;
}

void Z_Stats( zone_stats_t *stats ) {
	if ( stats ) {
		Com_Memset( stats, 0, sizeof( *stats ) );
		stats->zoneBytes = vmZoneUsed;
		stats->freeBytes = VM_ZONE_SIZE - vmZoneUsed;
	}
}

#else /* !Q3_VM -- full zone allocator below */
